...
const USER_ID_PATTERN = /^User ID\s*:?\s*(.+)$/im;
const SUBMITTED_AT_PATTERN = /^Submitted at\s*:?\s*(.+)$/im;
const CONTENT_BLOCK_PATTERN = /^(?:\[Content\]|Content)\s*:?\s*\n([\s\S]*?)(?:\n(?:\[Metadata\]|Metadata)\s*:?\s*\n?|\s*$)/im;
...
const isPotentialFeedbackReplySubject = (subject) => text(subject).toLowerCase().includes("[zeusian feedback]");
...
export async function syncNaverFeedbackReplies({ fetchCount, lookbackDays } = {}) {
  ...
  try {
    ...
    for (const message of messages) {
      const envelopeSubject = text(message?.envelope?.subject);
      if (!isPotentialFeedbackReplySubject(envelopeSubject)) {
        continue;
      }

      const parsed = await simpleParser(message.source);
      const subject = text(parsed?.subject || envelopeSubject);
