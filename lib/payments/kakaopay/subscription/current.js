import { buildCorsHeaders, getRuntimeConfig, sendJson } from "../../kakaopay.js";
import { authenticateSupabaseUserFromRequest } from "../../../billing/tier-sync.js";
import {
  buildPublicSubscription,
  fetchKakaoSubscriptionByUserId,
} from "../../../billing/subscription-store.js";

export default async function handler(req, res) {
  const { allowOrigin } = getRuntimeConfig(req);

  if (req.method === "OPTIONS") {
    res.writeHead(204, buildCorsHeaders(allowOrigin));
    res.end();
    return;
  }

  if (req.method !== "POST") {
    sendJson(res, 405, { message: "Method not allowed." }, allowOrigin);
    return;
  }

  const authResult = await authenticateSupabaseUserFromRequest(req);
  if (!authResult.ok) {
    sendJson(res, authResult.status, { message: authResult.message }, allowOrigin);
    return;
  }

  try {
    const subscription = await fetchKakaoSubscriptionByUserId({
      userId: authResult.userId,
      includeInactive: false,
    });
    sendJson(res, 200, { ok: true, subscription: buildPublicSubscription(subscription) }, allowOrigin);
  } catch (error) {
    sendJson(res, 500, { message: `Failed to load KakaoPay subscription: ${error.message}` }, allowOrigin);
  }
}
