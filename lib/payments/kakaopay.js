...
export const validateKakaoRuntimeConfig = ({ secretKey, cid, apiBase, requireCid = true }) => {
  ...
  if (!normalizedCid) {
    return requireCid
      ? "KAKAOPAY_CID is not set. Set the one-time payment CID before using one-time KakaoPay payments."
      : "";
  }
...
export const getRuntimeConfig = (req) => {
  ...
  const defaultCid = secretKey.startsWith("DEV") ? DEFAULT_CID : "";
  const cid = text(process.env.KAKAOPAY_CID) || defaultCid;
