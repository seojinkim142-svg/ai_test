...
import { redirectToPaymentClient } from "../../client-redirect.js";
...
const normalizeAuthPayload = (source = {}) => ({
  ...
  txTid: getFirstValue(source, ["TxTid", "txTid", "tid"]),
  authToken: getFirstValue(source, ["AuthToken", "authToken"]),
  amount: getFirstValue(source, ["Amt", "amount"]),
  signature: getFirstValue(source, ["Signature", "signature"]),
  responseMid: getFirstValue(source, ["MID", "mid"]),
  reservedToken: getFirstValue(source, ["ReqReserved", "reqReserved"]),
});
...
export default async function handler(req, res) {
  ...
  if (configError) {
    failAndRedirect(req, res, clientOrigin, configError);
    return;
  }
  ...
  try {
    if (req.method === "GET") {
      ...
      payload = normalizeAuthPayload(Object.fromEntries(requestUrl.searchParams.entries()));
      if (!hasAuthPayload(payload)) {
        failAndRedirect(req, res, clientOrigin, "결제가 취소되었습니다.");
        return;
      }
    } else {
      payload = normalizeAuthPayload(await parseRequestBody(req));
    }
  } catch {
  ...
  if (authResultCode !== "0000") {
    failAndRedirect(req, res, clientOrigin, authResultMsg || "Subscription billing authorization failed.");
    return;
  }

  const txTid = payload.txTid;
  const authToken = payload.authToken;
  const amount = payload.amount;
  const signature = payload.signature;
  const responseMid = payload.responseMid || billingMid;
  const reservedToken = payload.reservedToken;

  if (!txTid || !authToken || !amount || !signature || !reservedToken) {
    failAndRedirect(req, res, clientOrigin, "Required subscription billing fields are missing.");
    return;
  }

  if (responseMid !== billingMid) {
    failAndRedirect(req, res, clientOrigin, "Billing MID does not match.");
    return;
  }
  ...
  if (!safeEqual(signature.toLowerCase(), expectedSignature.toLowerCase())) {
    failAndRedirect(req, res, clientOrigin, "Subscription billing signature verification failed.");
    return;
  }
  ...
  if (!requestPayload || text(requestPayload?.flow) !== "nice_subscription") {
    failAndRedirect(req, res, clientOrigin, "Subscription request token is invalid or expired.");
    return;
  }
  ...
  if (!userId || !orderId || !requestedTier || !amountNumber) {
    failAndRedirect(req, res, clientOrigin, "Subscription request payload is invalid.");
    return;
  }
  ...
  if (!bid) {
    failAndRedirect(req, res, clientOrigin, "Bill key registration did not return BID.");
    return;
  }
  ...
  if (isProTrial) {
    ...
    if (!adminClient) {
      failAndRedirect(
        ...
        res,
        clientOrigin,
        "Server Supabase config is missing. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY."
      );
      return;
    }
    ...
    try {
      ...
      if (!grantResult.ok) {
        ...
        failAndRedirect(req, res, clientOrigin, grantResult.message || "Pro 무료 체험 시작에 실패했습니다.");
        return;
      }
      ...
      failAndRedirect(req, res, clientOrigin, error.message || "Pro 무료 체험 등록에 실패했습니다.");
      return;
    }
  }
