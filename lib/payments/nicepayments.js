...
const DEFAULT_SANDBOX_API_BASE = "https://sandbox-api.nicepay.co.kr";
const DEFAULT_PRODUCTION_API_BASE = "https://api.nicepay.co.kr";
...
const resolveDefaultApiBase = (origin) =>
  isLocalOrigin(origin) ? DEFAULT_SANDBOX_API_BASE : DEFAULT_PRODUCTION_API_BASE;
...
export const getRuntimeConfig = (req) => {
  ...
  const hostOrigin = normalizeOrigin(resolveHostOrigin(req));
  const vercelOrigin = normalizeOrigin(resolveVercelOrigin());
  const fallbackOrigin = hostOrigin || vercelOrigin || requestOrigin || DEFAULT_LOCAL_ORIGIN;
  const clientOrigin = normalizeOrigin(process.env.NICEPAYMENTS_CLIENT_ORIGIN) || fallbackOrigin;
  const apiBase =
    text(process.env.NICEPAYMENTS_API_BASE) ||
    resolveDefaultApiBase(clientOrigin || hostOrigin || vercelOrigin || requestOrigin || DEFAULT_LOCAL_ORIGIN);
  const subscribeApiBase =
    text(process.env.NICEPAYMENTS_SUBSCRIPTION_API_BASE || process.env.NICEPAYMENTS_API_BASE) ||
    resolveDefaultApiBase(clientOrigin || hostOrigin || vercelOrigin || requestOrigin || DEFAULT_LOCAL_ORIGIN);
...
export const buildNiceBasicAuthHeader = ({ clientId, secretKey }) => {
  const credentials = Buffer.from(`${text(clientId)}:${text(secretKey)}`, "utf8").toString("base64");
  return `Basic ${credentials}`;
};

export const requestNiceSubscribeApi = async ({ apiBase, path, payload, clientId, secretKey }) => {
  const response = await fetch(makeNiceApiUrl(apiBase, path), {
    method: "POST",
    headers: {
      Authorization: buildNiceBasicAuthHeader({ clientId, secretKey }),
...
export const formatNiceIsoDate = (dateInput = new Date()) => {
  const date = dateInput instanceof Date ? new Date(dateInput) : new Date(dateInput);
  return Number.isNaN(date.getTime()) ? "" : date.toISOString();
};

export const encryptNiceSubscribeCardData = ({
  secretKey,
  cardNumber,
  expYear,
  expMonth,
  ...
  const normalizedMode = text(encMode).toUpperCase();
  const plainText = `cardNo=${text(cardNumber)}&expYear=${text(expYear)}&expMonth=${text(expMonth)}&idNo=${text(idNo)}&cardPw=${text(cardPassword)}`;

  if (normalizedMode === "A2") {
    const key = Buffer.from(text(secretKey), "utf8");
    const iv = Buffer.from(text(secretKey).slice(0, 16), "utf8");
    const cipher = crypto.createCipheriv("aes-256-cbc", key, iv);
    return Buffer.concat([cipher.update(plainText, "utf8"), cipher.final()]).toString("hex");
  }

  const key = Buffer.from(text(secretKey).slice(0, 16), "utf8");
...
export const validateNiceSubscribeConfig = ({ clientId, secretKey, apiBase }) => {
  if (!text(clientId)) {
    return "NICEPAYMENTS_CLIENT_ID is not set.";
  }
  if (!text(secretKey)) {
    return "NICEPAYMENTS_SECRET_KEY is not set.";
  }
  if (!text(apiBase)) {
    return "NICEPAYMENTS_SUBSCRIPTION_API_BASE or NICEPAYMENTS_API_BASE is not set.";
  }
  return "";
};
