...
const DEFAULT_PROVIDER = "nicepayments";
...
export const isTestNiceSubscriptionClientId = (clientId) => /^nictest/i.test(text(clientId));

const getBillingAdminContext = () => {
...
export const buildPublicNiceSubscription = (row) => {
  if (!row) return null;

  const metadata = toPlainObject(row.metadata) || {};
  const cardNoMasked =
    text(metadata.cardNo || "") || text(row?.raw_charge?.CardNo || "") || text(row?.raw_approve?.CardNo || "");
  const cardName =
    text(metadata.cardName || "") || text(row?.raw_charge?.CardName || "") || text(row?.raw_approve?.CardName || "");
  const cardCode =
    text(metadata.cardCode || "") || text(row?.raw_charge?.CardCode || "") || text(row?.raw_approve?.CardCode || "");
  ...
  return {
    ...
    bidMasked: maskBid(row.sid),
    isTestClientKey: isTestNiceSubscriptionClientId(row.cid),
    isTestMid: isTestNiceSubscriptionClientId(row.cid),
    cardNoMasked,
    cardName,
    cardCode,
  };
...
export async function fetchNiceSubscriptionByUserId({ userId, includeInactive = true } = {}) {
...
export async function listDueNiceSubscriptions({ now = new Date(), limit = 20 } = {}) {
  const context = getBillingAdminContext();
  if (!context) {
    throw new Error("Server Supabase config is missing. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.");
  }
...
export async function upsertNiceSubscriptionRegistration({
  userId,
  bid,
  mid,
  tier,
  ...
  const normalizedUserId = text(userId);
  const normalizedBid = text(bid);
  const normalizedMid = text(mid);
  if (!normalizedUserId || !normalizedBid || !normalizedMid) {
    throw new Error("userId, bid, and mid are required.");
  }

  const existing = await fetchNiceSubscriptionByUserId({ userId: normalizedUserId, includeInactive: true });
  ...
  const payload = {
    ...
    next_charge_at:
      normalizeTimestamp(nextChargeAt) ||
      computeNextChargeAt({
...
export async function markNiceSubscriptionInactive({
  userId,
  reason = "",
  rawInactive = null,
}) {
...
export async function recordNiceSubscriptionChargeSuccess({
  userId,
  orderId = "",
  tid = "",
  rawCharge = null,
  ...
  const existing = await fetchNiceSubscriptionByUserId({ userId, includeInactive: true });
...
export async function recordNiceSubscriptionChargeFailure({
  userId,
  errorMessage = "",
  retryAfter = null,
  rawCharge = null,
